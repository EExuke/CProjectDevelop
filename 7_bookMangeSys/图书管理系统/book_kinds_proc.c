...
#define FIELD_NUM 1								//图书种类表的字段数
#define PRIMARY "bookkind"						//图书种类表的key
#define TB_NAME "tb_base_book_kind"					//图书种类表名
#define	EDIT_COUNT 1								//EDIT控件数量
#define	NO1_EDIT			IDC_BOOK_KINDS
BOOL CALLBACK Book_Kinds_Proc(HWND hDlg, UINT message,WPARAM wParam, LPARAM lParam)
{
	char titles[FIELD_NUM][TITLE_LENTH_MAX] = {"图书种类"};
	...
	case   WM_COMMAND :
		...
		{
		case IDC_BUTTON_ADD:
			sprintf(cms[0].fieldName,"bookkind");
			for(i=0;i<FIELD_NUM;i++)
			{
				cms[i].type = DB_CHAR;
			}
			GetDlgItemText(hDlg,IDC_BOOK_KINDS,cms[0].value,VALUE_LENTH_MAX);
			if(strlen(cms[0].value)<1)
			{
				MessageBox(hDlg,"请输入图书类名称","提示",MB_ICONHAND);
				return 0;
			}
			/*格式化字符串*/

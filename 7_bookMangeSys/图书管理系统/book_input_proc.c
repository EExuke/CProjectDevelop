...
#define FIELD_NUM 7								//窗体中对图书入库信息表可控制的字段数
#define PRIMARY "inputcode"						//主键
#define TB_NAME "tb_book_input"					//图书入库信息表名
...
#define BOOK_FK		"barcode"						//图书表外键
#define PROVIDER_FK	 "provider"						//供货商表外键
#define SUB_TITLES_BOOK_COUNT	8					//图书辅助窗口的标题数
#define SUB_TITLES_PROVIDER_COUNT	7				//供货商辅助窗口的标题数
...
BOOL CALLBACK Book_Input_Proc(HWND hDlg, UINT message,WPARAM wParam, LPARAM lParam)
{
	...
	int i;																 //迭代器
	char sets[(VALUE_LENTH_MAX+FIELD_LENTH_MAX)*(RESULT_TB_FIELD_NUM)+(RESULT_TB_FIELD_NUM-1)*2+1] = "\0";		 //Update语句中SET的内容，数组大小为：值和字段的总长度加上逗号、加号的数量
	...
	case   WM_COMMAND :
		...
		case IDC_BUTTON_ADD:
			...
			GetDlgItemText(hDlg,IDC_INPUT_BOOKNAME,cms[2].value,VALUE_LENTH_MAX);
		...
		case ID_QUIT:
			EndDialog(hDlg,0);
			break;
		case IDC_INPUT_BOOKNAME:
			if(HIWORD(wParam)==EN_CHANGE)
			{
				SetSubViewFromEdit(GetDlgItem(hDlg,IDC_INPUT_BOOKNAME),bookSubView,INFO_TB_BOOK_NAME,BOOK_FK,SUB_TITLES_BOOK_COUNT);
			}
			break;
	...
	case WM_NOTIFY:
		...
		if(NM_DBLCLK == pNmhdr->code)
		{
			...
			{
				SetWindowText(GetDlgItem(hDlg,IDC_INPUT_BOOKNAME),temp);
			}

...
#define TB_NAME "tb_book_sale_back"					//信息表名
...
{
	char titles[FIELD_NUM][TITLE_LENTH_MAX] = {"操作员","条形码","柜台","顾客姓名","退货数量","日期","实付金额"};
	...
	case   WM_COMMAND :
		...
		case IDC_BUTTON_ADD:
			...
			if(selIndex == -1)
			{
				MessageBox(hDlg,"请选择柜台","提示",MB_ICONHAND);
				return 0;
			}
			...
			if(strlen(temp)<1)
			{
				MessageBox(hDlg,"请输入折扣","提示",MB_ICONHAND);
				return 0;
			}
			rebate = atof(temp);
			if((int)(rebate*10)<3)
			{
				MessageBox(hDlg,"折扣过低","提示",MB_ICONHAND);
				return 0;
			}
			if((int)(rebate*10)>10)
			{
				MessageBox(hDlg,"折扣不能超过1","提示",MB_ICONHAND);
				return 0;
			}
